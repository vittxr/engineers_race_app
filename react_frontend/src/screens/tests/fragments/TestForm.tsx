...
const TestForm = ({ mutation, buttonText, isLoading }: Props) => {
  const [title, setTitle] = useState<string>('Criar registro de prova - Subida de Rampa em 45° (contagem de distância)')
  const [valueUnit, setValueUnit] = useState<'metros' | 'segundos' | 'gramas'>('metros')
  ...
  return (
    <>
      ...
      <RHForm id="test-form" onSubmit={mutation.mutateAsync} buttonText={buttonText}>
        <RHForm.Select label="Nome (tipo de prova)" id="name" 
          options={[
